...
使用的预设、尺寸和 NSFW 状态。参数回退、未识别中文或智能翻译结果会继续发送一条
`[提示]`；生成完成后再发送图片，生成失败时会发送具体原因。
...
`test_main.py` 覆盖命令参数、个人预设、个人 NSFW、冷却、即时反馈、越界编号和
转换失败恢复。`test_presets.py` 覆盖预设名解析、尺寸边界、提示词组装、负面词冲突剔除、
权重安全性与风格词冲突检查。

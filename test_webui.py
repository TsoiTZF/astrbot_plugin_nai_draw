...
def test_page_files():
    ...
    check((page / "app.js").is_file(), "存在外部脚本")
    check("./style.css" in html and "./app.js" in html, "页面使用相对资源路径")
    check('type="module"' not in html, "Dashboard iframe 不使用 ES module")
    ...
    check('class="studio-navbar"' in html and ".studio-navbar" in css, "顶栏类名与样式一致")
    check(".nav-menu-btn" in js and "nav-menu-btn" in html, "顶栏标签页选择器对齐")
    ...
    check('id="btn-random-artist"' in html and "artist-random-btn" in css, "绘台提供独立随机画师串按钮")
    check("random-artist" in js, "绘台随机画师串走后端接口")
